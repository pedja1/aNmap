...
public class Shell
{
    ...
    private static final ArrayList<FuseBind> M_FUSE_BINDs = new ArrayList<FuseBind>();
    ...
    private static Process spawnShell() throws IOException
    {
        return new ProcessBuilder().command("sh").start();
    }
     ...
     * @param receiver  the {@link rs.pedjaapps.anmap.core.Shell.OutputReceiver} that will receive command output
     ...
     * @param receiver  the {@link rs.pedjaapps.anmap.core.Shell.OutputReceiver} that will receive command output
     * @param withInput true if should create an output stream connected to the command stdin
     * @return {@link rs.pedjaapps.anmap.core.Shell.StreamGobbler} that keep reading from shell output until process dies,
    ...
    /**
     * calls {@link #async(String, rs.pedjaapps.anmap.core.Shell.OutputReceiver, boolean)}
    ...
    /**
     * calls {@link #async(String, rs.pedjaapps.anmap.core.Shell.OutputReceiver, boolean)}
     * with null {@link rs.pedjaapps.anmap.core.Shell.OutputReceiver}
    ...
    public static String getRealPath(final String file)
    {
        ...
        synchronized (M_FUSE_BINDs)
        {
            if (M_FUSE_BINDs.size() == 0)
                updateFuseBinds();
            if (M_FUSE_BINDs.size() > 0)
            {
                for (FuseBind b : M_FUSE_BINDs)
                {
